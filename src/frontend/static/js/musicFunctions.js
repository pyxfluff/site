...
async function showModal(artist, albumName, cover) {
  ...
  const averageRating = tracks.length
    ? Math.floor((totalRating / tracks.length) * 10) / 10
    : 0;
  ...
  if (albumName && albumName !== "") {
    ...
    if (summaryEl) {
      ...
      if (span)
        span.innerText = `${tracks[0] && tracks[0].year ? tracks[0].year : ""} • Average Rating: ${averageRating}`;
    }
  } else {
  ...
  if (songsEl) {
    if (albumName && albumName !== "") {
      songsEl.innerHTML = tracks
        .map((t, idx) => `<li>${idx + 1}. ${t.title} [${t.rating || 0}/5]</li>`)
        .join("");
    } else {
      songsEl.innerHTML = tracks
        .map(
          (t) =>
            `<li><img src="${t.cover}" alt="${t.album}" loading="lazy">${t.title} (${t.album} • ${t.rating || 0}/5)</li>`
        )
        .join("");
    }
  }
...
const initMusicPage = async () => {
  ...
  try {
    ...
    const tracks =
      (tracksData && tracksData.toptracks && tracksData.toptracks.track) || [];
    const albums =
      (albumsData && albumsData.topalbums && albumsData.topalbums.album) || [];
    ...
    if (searchInput && searchClear && tableBody) {
      const filterAndHighlight = () => {
        ...
        rows.forEach((row) => {
          ...
          const artist = (
            row.getAttribute("data-artist-name") || ""
          ).toLowerCase();
          const album = (
            row.getAttribute("data-album-name") || ""
          ).toLowerCase();
          const isMatch =
            song.includes(query) ||
          ...
          if (isMatch) {
            ...
            const cells = [
              row.querySelector(".song-info .title"),
              row.querySelector(".song-info .artist"),
              row.querySelector(".song-album")
            ];
            cells.forEach((cell) => {
              if (cell)
    ...
    const tableRows = document.querySelectorAll(
      ".music-challenge-table tbody tr"
    );
    ...
    if (repeatSongsList) {
      repeatSongsList.innerHTML = tracks
        .map((track, i) => {
          ...
          return `
            <div class="card song-card-mini song" data-embed-url="null">
      ...
      repeatSongsList
        .querySelectorAll(".card.song-card-mini.song")
        .forEach((el) => {
    ...
    if (repeatAlbumList) {
      repeatAlbumList.innerHTML = albums
        .map(
          (album) =>
            `<li class="song album">${album.name || album.title || ""}</li>`
        )
        .join("");
    }
  } catch (e) {
