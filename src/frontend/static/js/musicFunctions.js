...
async function showModal(artist, albumName, cover) {
    ...
    const ratingsMap = {};
    tracks.forEach((t) => { if (t.title)
        ratingsMap[t.title] = t.rating ?? 0; });
    ...
    if (albumName && albumName !== "") {
        if (titleText)
            titleText.textContent = `Details - ${albumName}`;
        if (modalCoverSmall)
            modalCoverSmall.src = cover || (tracks[0] && tracks[0].cover) || '';
        if (coverEl)
            coverEl.src = cover || (tracks[0] && tracks[0].cover) || '';
        if (summaryEl) {
            // omg shut up
            summaryEl.querySelector("h2").innerText = albumName;
            summaryEl.querySelector("h4").innerText = artist;
            summaryEl.querySelector("span").innerText = `${tracks[1].year} • Average Rating: ${Math.floor((totalRating / tracks.length) * 10) / 10}`; // i hope this is trustworthy
        }
    }
    else {
        if (titleText)
            titleText.textContent = `${artist} - Summary`;
        if (modalCoverSmall)
            modalCoverSmall.src = cover || (tracks[0] && tracks[0].cover) || '';
        if (coverEl)
            coverEl.src = cover || (tracks[0] && tracks[0].cover) || '';
        if (summaryEl) {
            ...
            summaryEl.querySelector("span").innerText = `Average rating: ${Math.floor((totalRating / tracks.length) * 10) / 10}`; // i hope this is trustworthy
        }
    }
    if (songsEl) {
        // improve for artist view
        if (albumName && albumName !== "") {
            songsEl.innerHTML = tracks.map((t, idx) => {
                return `<li>${idx + 1}. ${t.title} [${t.rating}/5]</li>`;
            }).join('');
        }
        else {
            songsEl.innerHTML = tracks.map((t) => {
...
const initMusicPage = (async () => {
    ...
    const newMusicChallengeData = (await (await fetch("/api/songs")).json())["2026"] ?? [];
    ...
    tableBody.addEventListener("click", (event) => {
        ...
        const albumCell = target.closest(".song-album");
        const artistCell = target.closest(".song-artist");
        if (!albumCell && !artistCell)
            return;
        const row = (albumCell ?? artistCell)?.closest("tr");
        if (!row)
            return;
        const artist = row.getAttribute("data-artist-name") ?? "", album = albumCell ? (row.getAttribute("data-album-name") ?? "") : "";
        void showModal(artist, album, row.querySelector(".song-cover img")?.src ?? "");
    });
    ...
    cards.forEach(card => {
        card.addEventListener("click", () => {
            ...
            document.querySelector(".music-display").style.position = "absolute";
            if (embedUrl) {
                document.querySelector(".music-display .embed").innerHTML = `<iframe style="border-radius:12px" src="${embedUrl}" width="100%" height="352" frameBorder="0" allowfullscreen="" allow="autoplay; clipboard-write; encrypted-media; fullscreen; picture-in-picture" loading="lazy"></iframe>`;
                document.querySelector(".music-display").classList.remove("hidden");
            }
            else {
...
(async () => {
    document.querySelector(".music-display").addEventListener("click", async () => {
        document.querySelector(".music-display").classList.add("hidden");
        // force-stop the player
        document.querySelector(".music-display .embed").innerHTML = "";
    });
    await initMusicPage();
})();
