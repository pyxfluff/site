...
async function setRecentlyPlaying() {
  let res = await fetch(
    "https://ws.audioscrobbler.com/2.0/?method=user.getrecenttracks&user=pyxfluff&api_key=974a5ebc077564f72bd639d122479d4b&limit=1&page=1&format=json"
  );
  ...
  if (songMeta)
    songMeta.querySelector("span").innerText =
...
(async () => {
  ...
  document.querySelectorAll(".nav button").forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      document
        .querySelectorAll(".nav button, .tab-pane")
  ...
  if (
    new URLSearchParams(window.location.search).get("isOldDomain") == "true"
  ) {
