...
async function setRecentlyPlaying() {
    ...
    if (songName)
        songName.querySelector("span").innerText = track.name;
    if (songMeta)
        songMeta.querySelector("span").innerText = `${track.artist["#text"]} · ${track.album["#text"]}`;
    if (artStyle) {
    ...
    if (priorListener)
        document.querySelector(".song-card")?.removeEventListener("click", priorListener);
...
document.querySelector(".img-card .pfp")?.addEventListener("click", () => {
    ...
    x = false;
    document.querySelector(".img-card .pfp").src = art[Math.floor(Math.random() * 3)];
});
if (window.lucide)
    lucide.createIcons();
